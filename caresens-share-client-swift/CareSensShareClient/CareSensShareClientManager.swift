//
//  CareSensShareClientManager.swift
//  Loop
//
...
public class CareSensShareClientManager: CGMManager {

    public static let pluginIdentifier = "DexCareSensShareClient"
    ...
    public var shareService: ShareService {
        didSet {
            try! keychain.setCareSensShareUsername(shareService.username, password: shareService.password, url: shareService.url)
        }
    }

    public let localizedTitle = LocalizedString("CareSens Share", comment: "Title for the CGMManager option")
    ...
    public func fetchNewDataIfNeeded(_ completion: @escaping (CGMReadingResult) -> Void) {
        guard let CareSensShareClient = shareService.client else {
            completion(.noData)
            return
        }
        ...
        CareSensShareClient.fetchLast(6) { (error, glucose) in
            if let error = error {
...
extension CareSensShareClientManager {
