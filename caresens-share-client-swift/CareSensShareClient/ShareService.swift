...
// Encapsulates the CareSens Share client service and its authentication
public class ShareService: ServiceAuthentication {
    public var credentialValues: [String?]

    public let title: String = LocalizedString("CareSens Share", comment: "The title of the CareSens Share service")
    ...
    public init(username: String?, password: String?, url: URL?) {
        ...
        */
        ...
        if let username = username, let password = password, let url = url {
            isAuthorized = true
            client = CareSensShareClient(username: username, password: password, shareServer: url.absoluteString)
        }
    }

    // The share client, if credentials are present
    private(set) var client: CareSensShareClient?
    ...
    public func verify(_ completion: @escaping (_ success: Bool, _ error: Error?) -> Void) {
        ...
        let client = CareSensShareClient(username: username, password: password, shareServer: url.absoluteString)
...
extension KeychainManager {
    func setCareSensShareUsername(_ username: String?, password: String?, url: URL?) throws {
        ...
        try replaceInternetCredentials(nil, forURL: CareSensShareURL)

        try replaceInternetCredentials(credentials, forLabel: CareSensShareServiceLabel)
    }

    func getCareSensShareCredentials() -> (username: String, password: String, url: URL)? {
        do { // Silence all errors and return nil
            do {
                let credentials = try getInternetCredentials(label: CareSensShareServiceLabel)
                ...
                let credentials = try getInternetCredentials(url: CareSensShareURL)

                try setCareSensShareUsername(credentials.username, password: credentials.password, url: credentials.url)
...
extension ShareService {
    public convenience init(keychainManager: KeychainManager = KeychainManager()) {
        if let (username, password, url) = keychainManager.getCareSensShareCredentials() {
